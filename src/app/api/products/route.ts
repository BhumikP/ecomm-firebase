...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // Validate images and determine primary image
        const imagesToSave = Array.isArray(body.images) && body.images.length > 0
            ? body.images.map(img => String(img).trim()).filter(img => img)
            : (body.image ? [String(body.image).trim()].filter(img => img) : []);

        if (imagesToSave.length === 0) {
             return NextResponse.json({ message: 'At least one valid image URL is required.' }, { status: 400 });
        }
        ...
        if (body.colors && Array.isArray(body.colors)) {
            for (const color of body.colors) {
                ...
                for (const imageIndex of color.imageIndices) {
                    // Validate that imageIndex is a non-negative integer within bounds
                ...
                parsedColors.push({
                    ...
                    stock: color.stock
                    // _id will be generated by Mongoose for new subdocuments
                });
            }
        }
        ...
        const newProductData = {
            ...
            images: imagesToSave, // Set the full list of images
         ...
         // Remove the original 'colors' if it existed in the raw body to avoid confusion
        // delete (newProductData as any).colors;
