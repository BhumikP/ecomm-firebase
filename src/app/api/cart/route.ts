...
export async function GET(req: NextRequest) {
  ...
  try {
    const cart = await Cart.findOne({ userId }).populate({
      ...
      select: 'title price thumbnailUrl stock colors minOrderQuantity category subcategory rating discount', // Select necessary fields
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (quantity < minOrderQty) {
        return NextResponse.json({ message: `Minimum order quantity for this product is ${minOrderQty}.` }, { status: 400 });
    }
    ...
    const availableStock = selectedColorData ? selectedColorData.stock : product.stock;
    ...
    if (existingItemIndex > -1) {
      // Update quantity and snapshots of existing item
      cart.items[existingItemIndex].quantity = quantity;
      cart.items[existingItemIndex].priceSnapshot = priceSnapshot;
      cart.items[existingItemIndex].imageSnapshot = imageSnapshot;
      cart.items[existingItemIndex].nameSnapshot = product.title;
    } else {
      // Add new item
      const newItemData: Partial<ICartItem> = { // Use Partial because _id is auto-generated by Mongoose for subdocuments
      ...
      if (selectedColorData) {
        newItemData.selectedColorSnapshot = {
          name: selectedColorData.name,
          hexCode: selectedColorData.hexCode,
        };
      }
      cart.items.push(newItemData as ICartItem);
    }

    await cart.save();
    const populatedCart = await Cart.findById(cart._id).populate({
        path: 'items.product', model: 'Product', select: 'title price thumbnailUrl stock colors minOrderQuantity discount'
    });
